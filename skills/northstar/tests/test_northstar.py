...
class NorthstarTests(unittest.TestCase):
    ...
    def test_claim_is_exclusive_and_requires_ready(self):
        self.add_ready()
        northstar.claim_item(
            args(root=self.root, item="RM-001", owner="Maya", branch="feat/rm-001-invites", wayfinder="roadmap/maps/RM-001.md")
        )
        with self.assertRaises(northstar.NorthstarError):
            northstar.claim_item(
                args(root=self.root, item="RM-001", owner="Iker", branch="feat/duplicate", wayfinder="roadmap/maps/RM-001.md")
            )
        item = northstar.Roadmap.load(self.root / "ROADMAP.md").find("RM-001")
        self.assertEqual((item["Status"], item["Owner"]), ("In Progress", "Maya"))
    ...
    def test_close_requires_acceptance_and_graphify_evidence(self):
        self.add_ready()
        northstar.claim_item(
            args(root=self.root, item="RM-001", owner="Maya", branch="feat/rm-001-invites", wayfinder="roadmap/maps/RM-001.md")
        )
        with self.assertRaises(northstar.NorthstarError):
            northstar.close_item(args(root=self.root, item="RM-001", graphify="Updated: graphify-out", evidence="PR #42"))
        brief = self.root / "roadmap" / "items" / "RM-001.md"
        northstar.atomic_write(brief, brief.read_text().replace("- [ ]", "- [x]"))
        northstar.close_item(args(root=self.root, item="RM-001", graphify="Updated: graphify-out at abc123", evidence="PR #42"))
    ...
    def test_sync_state_does_not_replace_work_status(self):
        ...
        item["Sync"] = northstar.sync_state([
            {"service": "github", "status": "ok"},
            {"service": "gitlab", "status": "error"},
        ])
        roadmap.save()
        updated = northstar.Roadmap.load(self.root / "ROADMAP.md").find("RM-001")
        self.assertEqual(updated["Status"], "Ready")
        self.assertEqual(updated["Sync"], "Partial")

    def test_update_reprioritizes_and_records_reason(self):
        self.add_ready()
        northstar.update_item(
            args(root=self.root, item="RM-001", priority="P0", status="Deferred", title=None, reason="Dependency moved to next quarter")
        )
        item = northstar.Roadmap.load(self.root / "ROADMAP.md").find("RM-001")
